...
from src.clients.apollo import search_people, enrich_people, find_leads_at_company
...
async def test_search_people_returns_results():
    ...
    with patch("src.clients.apollo.APOLLO_API_KEY", "test-key"):
        with patch("src.clients.apollo.httpx.AsyncClient") as mock_client:
...
async def test_search_people_no_api_key_returns_empty():
    ...
    with patch("src.clients.apollo.APOLLO_API_KEY", ""):
...
async def test_enrich_people_returns_emails():
    ...
    mock_response = MagicMock()
    # API returns {"person": {...}} for each match call
    mock_response.json.return_value = {
        "person": {
            "id": "abc123",
            "first_name": "John",
...
async def test_find_leads_at_company_returns_leads_with_email():
    ...
    with patch("src.clients.apollo.search_people") as mock_search:
        with patch("src.clients.apollo.enrich_people") as mock_enrich:
